    ...
    for (const it of items) {
      ...
      if (it.isDirectory()) {
        ...
        for (const c of children) {
          ...
          entries.push({ path: cfull, name: path.join(it.name, c.name), isDirectory: c.isDirectory() });
        }
      } else {
...
ipcMain.handle('terminal:create', async (event, { cols = 80, rows = 24, shell } = {}) => {
  const shellCmd = shell || (process.platform === 'win32' ? 'powershell.exe' : (process.env.SHELL || 'bash'));
  ...
  if (pty) {
    ...
    term.on('data', (data) => {
      try { event.sender.send('terminal:data', { id, data }); } catch (e) {}
    });
    term.on('exit', (exitCode, signal) => {
      try { event.sender.send('terminal:exit', { id, exitCode, signal }); } catch (e) {}
      terminals.delete(id);
    });
  ...
  try {
    ...
    const child = cp.spawn(shellCmd, [], { cwd: process.cwd(), env: process.env, shell: true });
    // provide a minimal terminal-like wrapper
    const wrapper = {
      write: (d) => { try { child.stdin.write(d); } catch (e) {} },
      resize: (c, r) => { /* noop - no PTY */ },
      kill: () => { try { child.kill(); } catch (e) {} }
    };
    terminals.set(id, wrapper);

    child.stdout.on('data', (chunk) => { try { event.sender.send('terminal:data', { id, data: String(chunk) }); } catch (e) {} });
    child.stderr.on('data', (chunk) => { try { event.sender.send('terminal:data', { id, data: String(chunk) }); } catch (e) {} });
    child.on('close', (code) => { try { event.sender.send('terminal:exit', { id, exitCode: code }); } catch (e) {} ; terminals.delete(id); });
