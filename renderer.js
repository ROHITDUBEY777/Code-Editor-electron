...
function getLangFromPath(p) {
  ...
  const map = { js: 'javascript', ts: 'typescript', json: 'json', html: 'html', css: 'css', md: 'markdown' };
...
function createTab({ filePath, name, content }) {
  ...
  const uri = filePath ? monaco.Uri.file(filePath) : monaco.Uri.parse('inmemory:///' + name + id + '.' + language);
  const model = monaco.editor.createModel(content || '', language, uri);
  const tab = { id, filePath, name: name || (filePath ? requireName(filePath) : 'untitled'), content, model, isDirty: false };
...
async function openFileDialog() {
  ...
  if (res && !res.canceled) {
    createTab({ filePath: res.filePath, name: res.filePath.split(/[\\/]/).pop(), content: res.content });
  }
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('saveBtn').addEventListener('click', async () => {
    ...
    if (tab.filePath) {
      ...
      if (w && w.ok) { tab.isDirty = false; renderTabs(); }
...
function renderRootFolder(rootPath) {
  const tree = document.getElementById('fileTree');
  tree.innerHTML = '';
  const rootNode = createTreeNode({ name: rootPath, path: rootPath, isDirectory: true }, 0);
...
function toggleTerminal() {
  ...
  if (!panel.classList.contains('hidden')) {
    ...
    setTimeout(() => { if (fitAddon) fitAddon.fit(); }, 120);
  }
}
...
async function createTerminal(killExisting = false) {
  ...
  if (killExisting && terminalId) {
    try { await window.terminalAPI.kill(terminalId); } catch (e) {}
    if (term) { try { term.dispose(); } catch (e) {} }
    term = null; terminalId = null;
  }
  ...
  const FitCtor = window.FitAddon || (window.FitAddon && window.FitAddon.FitAddon) || window.FitAddon;
  if (!TerminalCtor) {
    const status = document.getElementById('terminalStatus');
    if (status) status.textContent = 'xterm.js not loaded (check CDN/network)';
    const panel = document.getElementById('terminal-panel'); if (panel) panel.classList.remove('hidden');
    return;
  }

  term = new TerminalCtor({ cursorBlink: true, fontFamily: 'monospace' });
  if (typeof FitCtor === 'function') {
    try { fitAddon = new FitCtor(); if (typeof term.loadAddon === 'function') term.loadAddon(fitAddon); } catch (e) { fitAddon = null; }
  }
  term.open(container);
  ...
  window.terminalAPI.onExit(({ id, exitCode }) => {
    if (id === terminalId && term) term.write('\r\n[process exited ' + exitCode + ']\r\n');
  });
  ...
  setTimeout(() => {
    try { if (fitAddon && term) { fitAddon.fit(); if (terminalId) window.terminalAPI.resize(terminalId, term.cols, term.rows); } } catch (e) {}
  }, 120);
  ...
  window.addEventListener('resize', () => {
    ...
    resizeTimer = setTimeout(() => {
      try {
        if (fitAddon && term) { fitAddon.fit(); window.terminalAPI.resize(terminalId, term.cols, term.rows); }
      } catch (e) {}
    }, 150);
  });
}
